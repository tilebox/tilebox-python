...
class DatasetClient:
    ...
    def collections(self, availability: bool | None = None, count: bool | None = None) -> dict[str, "CollectionClient"]:
        """
    ...
    def get_or_create_collection(self, name: str) -> "CollectionClient":
    ...
    def create_collection(self, name: str) -> "CollectionClient":
    ...
    def find(
        ...
        """
        collection_ids = self._collection_ids(collections)
        try:
            datapoint = self._service.query_by_id(
                self._dataset.id,
                collection_ids,
    ...
    def _collection_id(self, collection: "UUID | Collection | CollectionInfo | CollectionClient") -> UUID:
        if isinstance(collection, CollectionClient):
    ...
    def _collection_ids(
        ...
        all_collections: list[CollectionInfo] = self._service.get_collections(self._dataset.id, True, True).get()
...
class CollectionClient:
        ...
        *,
        ...
        with tqdm(
            ...
            for chunk_start in range(0, len(messages), _INGEST_CHUNK_SIZE):
                ...
                response = self._dataset._service.ingest(self._collection.id, chunk, allow_existing).get()
    ...
    def delete(self, datapoints: DatapointIDs, *, show_progress: bool | ProgressCallback = False) -> int:
        ...
        with tqdm(
            ...
            for chunk_start in range(0, len(datapoint_ids), _DELETE_CHUNK_SIZE):
                ...
                num_deleted += self._dataset._service.delete(self._collection.id, chunk).get()
